    ...
    if request.method == "POST":
        ...
        if float(user_selected_plan.price) <= 1:
            messages.add_message(
                request,
                messages.INFO,
                f"Please contact to administrator to get access selected plan <{user_selected_plan.name}>",
            )
    ...
    if User.objects.filter(username=username):
        if Subscriber.objects.filter(user=request.user):
            ...
            messages.add_message(
                request,
                messages.WARNING,
                "Seams like you don't have subscription with us please select one of the plans",
            )
            return redirect(reverse("main"))
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                feature = Feature.objects.create(
                    feature_type="Basic", user=related_user
                )
...
def dashboard_view(request):
    ...
    if Subscriber.objects.filter(user=current_user):
        ...
        messages.add_message(
            request,
            messages.INFO,
            "You don't have the subscription with us please choice one of the subsciption!!",
        )
        return redirect(reverse("main"))
